...
impl<T> UserRingBufferSample<'_, T> {
    /// Retrieve a mutable reference the sample of type `T` within the ring buffer.
    ///
...
impl UserRingBuffer {
    ...
    ///
    /// Returns a [`UserRingBufferSample<T>`](UserRingBufferSample<'self,T>)
    /// that contains a mutable reference to a type T that can be written to.
